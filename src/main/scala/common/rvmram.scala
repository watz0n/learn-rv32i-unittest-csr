        ...
        when(io.data_port.req.valid) {
            switch(io.data_port.req.mfunc) {
                is(mram_op.MF_WR) {
                    ...
                    when(io.data_port.req.addr(31) === 1.U) {
                        ahmem.write(addr, data_vec, data_mask)
                    }
                    .otherwise {
                ...
                is(mram_op.MF_RD) {
                    ...
                    //io.data_port.resp.data := amem.read(addr).asUInt
                    when(io.data_port.req.addr(31) === 1.U) {
                        io.data_port.resp.data := ahmem.read(addr).asUInt
                    }
                    .otherwise {
                        io.data_port.resp.data := amem.read(addr).asUInt
                    }
        ...
        when(io.dbg_port.req.valid) {
            switch(io.dbg_port.req.mfunc) {
                is(mram_op.MF_WR) {
                    ...
                    */
                    //amem.write(addr, data_vec, data_mask)
                    when(io.dbg_port.req.addr(31) === 1.U) {
                        ahmem.write(addr, data_vec, data_mask)
                    }
                    .otherwise {
                        amem.write(addr, data_vec, data_mask)
                ...
                is(mram_op.MF_RD) {
                    ...
                    when(io.dbg_port.req.addr(31) === 1.U) {
                        io.dbg_port.resp.data := ahmem.read(addr).asUInt
                    }
                    .otherwise {
                        io.dbg_port.resp.data := amem.read(addr).asUInt
                    }
